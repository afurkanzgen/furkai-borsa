...
def app(environ, start_response):
    ...
    try:
        if method=='GET':
            ...
            if path=='/api/portfolio': return _json(start_response,{'ok':True,'portfolio':server.load_portfolio()})
            if path=='/api/portfolio/intelligence': return _json(start_response,server.portfolio_intelligence())
        ...
        if method=='POST':
            ...
            if path=='/api/portfolio/save': server.save_portfolio(body.get('portfolio',[])); return _json(start_response,{'ok':True,'portfolio':server.load_portfolio()})
            if path=='/api/config': return _json(start_response,server.save_config(body))
            ...
            if path=='/api/scan': return _json(start_response,server.scan(body))
